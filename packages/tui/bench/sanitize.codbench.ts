/**
 * CodSpeed/tinybench variant of `sanitize.ts`.
 *
 * One task per sanitizeText variant under investigation (regex + Variants A–E).
 * Each task runs makeBench-style fixed iteration (`ITERATIONS` inner loop) over
 * the full sample set so a single tinybench op amortizes the per-call cost of
 * these microsecond-scale functions. Six tasks:
 *   - regexSanitizeText            — naive single-regex strip.
 *   - gatedSanitizeText             — (A) cheap regex gate → current fallback.
 *   - skipRunSanitizeText           — (B) regex.exec driven, skip clean runs.
 *   - removalStartSanitizeText      — (C) regex matches only real removal starts.
 *   - wellFormedControlSanitizeText — (D) skip surrogate matches when well-formed.
 *   - lazyWellFormedSanitizeText    — (E) broad scan, isWellFormed only on pair.
 *
 * Correctness (each variant vs the current production sanitizeText) is verified
 * before timing; a mismatch aborts. Warmup is left to tinybench.
 */
...
const sampleList: string[] = Object.values(samples);

const variants: Record<string, (text: string) => string> = {
	regexSanitizeText,
	gatedSanitizeText,
	skipRunSanitizeText,
	removalStartSanitizeText,
	wellFormedControlSanitizeText,
	lazyWellFormedSanitizeText,
};

// Verify each variant matches the current production sanitizeText.
let mismatches = 0;
for (const [name, fn] of Object.entries(variants)) {
	for (const [sampleName, text] of Object.entries(samples)) {
		const baseline = currentSanitizeText(text);
		const result = fn(text);
		if (result !== baseline) {
			console.log(`MISMATCH ${name}/${sampleName}: result=${JSON.stringify(result)} baseline=${JSON.stringify(baseline)}`);
			mismatches++;
		}
	}
}
if (mismatches > 0) {
	...
	console.log(`sanitize: all ${Object.keys(variants).length} variants match current on ${sampleList.length} samples.\n`);
}

// ─── Bench ──────────────────────────────────────────────────────────────────
// makeBench-style: each task runs ITERATIONS passes over the full sample set
// inside one tinybench op, amortizing the per-call cost of these
// microsecond-scale functions.
...
for (const [name, fn] of Object.entries(variants)) {
	bench.add(name, () => {
		for (let i = 0; i < ITERATIONS; i++) {
			for (const text of sampleList) fn(text);
		}
	});
}
