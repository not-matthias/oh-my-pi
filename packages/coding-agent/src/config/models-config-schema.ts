...
/**
 * Builds the full models/provider config schema graph.
 *
 * Every `type()` definition below is expensive (schema parsing + the jitless
 * interpreted-traversal tables), yet the resulting schema is only ever needed
 * when a config file is actually loaded or defaulted — never at import time.
 * Wrapping the whole graph in a factory deferred behind single-slot
 * memoization ({@link getModelsConfigSchema} and friends) moves ~97ms of
 * construction off the module-load critical path: importing this module is now
 * free, and the schemas are built lazily on first use and then cached.
 *
 * `ConfigFile` accepts the schema as either an eager `Type` or a `() => Type`
 * factory, so `models-config.ts` passes the factory and pays nothing at import.
 */
function makeModelsConfigSchema() {
...
let _schemaBundle: ReturnType<typeof makeModelsConfigSchema> | undefined;
function schemaBundle() {
	if (!_schemaBundle) _schemaBundle = makeModelsConfigSchema();
	return _schemaBundle;
}

export function getModelsConfigSchema() {
	return schemaBundle().ModelsConfigSchema;
}

export function getOpenAICompatSchema() {
	return schemaBundle().OpenAICompatSchema;
}

export function getModelOverrideSchema() {
	return schemaBundle().ModelOverrideSchema;
}

export function getProviderDiscoverySchema() {
	return schemaBundle().ProviderDiscoverySchema;
}

export function getProviderAuthSchema() {
	return schemaBundle().ProviderAuthSchema;
}

export type ModelsConfig = ReturnType<typeof getModelsConfigSchema>["infer"];
export type ModelOverride = ReturnType<typeof getModelOverrideSchema>["infer"];
export type ProviderAuthMode = ReturnType<typeof getProviderAuthSchema>["infer"];
export type ProviderDiscovery = ReturnType<typeof getProviderDiscoverySchema>["infer"];
